...
/*
    private void keepCalm ( SoundWalkComponent walkSoundComponent ) {
        direction.set( 0, 0 );

        if ( walkSoundComponent == null ) {
            return;
        }
        walkSoundComponent.walk.setVolume( walkSoundComponent.id, 0 );
        //walk.setThisVolume( 0.0f );
    }



    private void soundByDistance ( SoundWalkComponent walkSoundComponent ) {
        if ( walkSoundComponent == null ) {
            return;
        }
        float MAX_LISTEN_DISTANCE = 20.0f;
        // Set NPC step-sounds by distance of player
        Vector2 playerPosition = new Vector2( Core.playerPosition.x, Core.playerPosition.z );//MyPlayer.getPosition();
        ...
        walkSoundComponent.walk.setVolume( walkSoundComponent.id, walkVolume );
    }
*/
